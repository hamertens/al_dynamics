...
# Define the BNN
def train_dropout_model(x_train, y_train, x_test, y_test, input_dim = INPUT_DIMENSIONALITY, output_dim = OUTPUT_DIMENSIONALITY):
  ...
  losshistory, train_state = BNN_model.train(iterations=15000, callbacks= [BNN_uncertainty])
  del losshistory
  del data
  del net
...
def check_divergence(metrics_df):
    ...
    average_rmse_old = np.mean(rmse_array[-510:-500])
    if average_rmse_current*1.01 > average_rmse_old:
        return True
    return False