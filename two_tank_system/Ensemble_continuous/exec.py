...
from setup import DATAFRAME_COLUMNS_INPUT, DATAFRAME_COLUMNS_OUTPUT, DATA_FILEPATH, FOLDER_FILEPATH
...
for test_model, train_model in zip(model.test_estimators, model.train_estimators):
    ...
    train_model.compile(loss=regression_gaussian_nll_loss(test_model.get_layer(last_layer_name).output), optimizer='adam')
...
predictions_df = pd.DataFrame(model_predictions, columns= DATAFRAME_COLUMNS_OUTPUT)
